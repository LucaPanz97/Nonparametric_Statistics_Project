...
# Below some permutational t-test on the two variables that were more likely to 
# be not significative because of their higher pvalues on the summary

#t-test on log10.sqm_lot.
...
treemod = train_XGB(x_train[4:34],y_train,x_test[4:34],y_test)
...
hist(as.numeric(unlist(unname(datagg[,2]))), breaks=20, main="MAE distribution") 
...
datagg_pct = data_pct[,2:3] %>% 
  group_by(group) %>% 
  summarise(across(everything(), list(mean = mean)))
plot(datagg_pct, pch=20, xlab="bins", ylab="MAPE", main="Average MAPE - houses grouped in 100 bins")
abline(v=94, col="red")
...
# Distribution of pct error (having the mean error is not satisfying)
hist(as.numeric(unlist(unname(datagg_pct[,2]))), breaks=20, main="MAPE distribution") 
...
final_mae  # 66k on standard 200k on expensive
